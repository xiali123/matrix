...
__global__ void copy_const_kernel(float* d)
{
...
void anim_gpu(uchar4* output_bitmap, DataBlock* d, int ticks)
{
	...
	for (int i = 0; i < 90; i++)
	{
		...
		blend_kernel << <blocks, threads >> > (out, d->dev_constStr);
		dstOut = !dstOut;
	}
...
int main()
{
	...
	temp[DIM * 400 + 400] = MIN_TEMP;
	temp[DIM * 600 + 200] = MIN_TEMP;
	...
	cudaMemcpy(data.dev_constStr, temp, DIM * DIM * sizeof(float), cudaMemcpyHostToDevice);
	...
	cudaMemcpy(data.dev_inStr, temp, DIM * DIM * sizeof(float), cudaMemcpyHostToDevice);
